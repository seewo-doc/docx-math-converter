...
export async function convertLatex2Math(latexString: string) {
  const mathMlString = await latex2MathMl(latexString);
  return convertMathMl2Math(mathMlString);
}

async function latex2MathMl(latexString: string) {
